...
The catch here is the distribution is not stationary meaning that the 
parameter of the distribution may not be same as previous step.
To maximize the reward the values are inintialized according to optimal 
initialization.
"""
import math
from typing import *
...
def run_experiments(
    k_distribution, steps: int, initial_value: float, epsilon: float
    ...
    def alpha(x):
        return math.sin(n_of_actions[x]) ** 2
    ...
    for _ in range(steps):
        ...
        values_of_actions[choice] += (reward - values_of_actions[choice]) * alpha(
            choice
        )
